...
class ImportFiles:
    def import_files(
        ...
        **kwargs,
        ...
        self._check_input_files(edge_csv, label_id)
    ...
    def _check_input_files(self, edge_csv: str, label_id: str) -> bool:
        """
        ...
        if edge_csv == "./inputs/neighbour_matrix.csv":
            ...
            if not file_exists:
                self._generate_random_graph_edges(left_nodes_count, right_nodes_count)
        if label_id == "./inputs/id_labels.csv":
            file_exists = os.path.isfile("./inputs/id_labels.csv")
            if not file_exists:
                self._generate_random_graph_labels(left_nodes_count, right_nodes_count)
        return True

    def _generate_random_graph_labels(
        self, left_nodes_count: int, right_nodes_count: int
    ) -> bool:
        """
    ...
    def _generate_random_graph_edges(
        self, left_nodes_count: int, right_nodes_count: int
    ) -> bool:
        """
